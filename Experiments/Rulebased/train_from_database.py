...
class PoolOfStates(torch.utils.data.IterableDataset):
    ...
    def _yield_dict(self):
        cursor = self._connection.cursor()
        action = "" if self._drop_actions else ", int_action, dict_action"
        obs_dict = {}
        cursor.execute(f'SELECT agent, current_player, '
...
def int_action_to_dict(int_action):
    ...
    color = -1
    rank = -1
    if 0 <= int_action <= 4: action_type = 'PLAY'
    elif 5 <= int_action <= 9: action_type = 'DISCARD'
    elif 10 <= int_action <= 29: action_type = 'REVEAL_COLOR'
    elif 30 <= int_action <= 49: action_type = 'REVEAL_RANK'
    ...
    return {'action_type': action_type, 'card_index': card_index, 'target_offset': target_offset, 'color': color, 'rank': rank}
...
def train_eval(config,
    ...
    for state in dataloader:
        try:
            ...
            if state['agent'] == str(target_agent_cls):
